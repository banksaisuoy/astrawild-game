...
namespace
{
        // ASSET OVERHAUL (fetch_free_assets.py): the species holding a REAL
        // unique CC0 mesh under ArtSource/Meshes/Echoes/SK_Echo_<Name>.glb —
        // 33 Tier-B species drawn 1:1 from the Quaternius CC0 creature pool
        // (theme-aware queues, deterministic order) + the 3 production boss
        // species (DrownedSovereign / EyeSentinel / GlassTyrant, Act-3 chain).
        // The remaining bestiary species render via their theme's REAL base
        // mesh (SK_Base_*, 16 unique CC0 models) + their unique deterministic
        // mutation spec — real geometry + real mutation, never a palette swap.
        // Mirrors ArtSource/manifest.json (status present) — the static
        // validator + ASTRAWILD.PCR4.TierBLibrary verify every entry.
        const TArray<FName>& GetTierBSpeciesTable()
        {
                static const TArray<FName> Table =
                {
                        ...
                        TEXT("Echo_Coralray"), TEXT("Echo_DrownedSovereign"), TEXT("Echo_Duskmoth"),
                        TEXT("Echo_Eldermonolith"), TEXT("Echo_Emberfang"), TEXT("Echo_Embershade"),
                        TEXT("Echo_EyeSentinel"), TEXT("Echo_Fernthorn"), TEXT("Echo_Forgottencolossus"),
                        TEXT("Echo_Frostblaze"), TEXT("Echo_Geargolem"), TEXT("Echo_Ghostshade"),
                        TEXT("Echo_GlassTyrant"), TEXT("Echo_Glimmerhornet"), TEXT("Echo_Hallowedcolossus"),
