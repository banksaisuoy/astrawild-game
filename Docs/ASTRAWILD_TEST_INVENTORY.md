...
**Suite**: 133 world-free contract tests · `Source/AstrawildCore/Private/AstrawildAutomationTests.cpp`
...
## LCP-3 LAN co-op contracts (112-113, landed in the LCP-3 batch)
...
## LCP-5 LAN co-op contracts (116-117, landed in the LCP-5 batch)
...
| 121 | ASTRAWILD.PCR2.RosterScreen | PCR-2 (PG-2): Echo Roster screen — the party-ring eligibility predicate (captured + valid identity + NOT benched — one pure rule for the spawn path and the screen), ring capacity rules (bench always allowed, deploy needs a free slot), and the wiring contract (controller ToggleRosterScreen/IsRosterOpen/RosterMirror replication/Request+ServerSetEchoBenched, character RosterAction, pause-menu RosterButton, subsystem SetInstanceBenched, row bBenched additive field with legacy-unbenched default). Closes the LCP-5 "no client roster UI" gap and fixes the ExportForSave ownership-strip defect (live actor refresh used to reset OwnerPlayerKey to NAME_None, orphaning co-op parties after save/load) |

| 122 | ASTRAWILD.PCR3.MapScreen | PCR-3 (PG-3): World Map screen — the pure world→map projection (uniform scale with centered letterbox so the 4:3 world never stretches; corners/center/quarter map proportionally; degenerate bounds fail closed to the corner instead of dividing by zero), and the wiring contract (controller ToggleMapScreen/IsMapOpen, character MapAction, pause-menu MapButton, WorldEventSubsystem GetActiveRuntimeEvents for event pins, POISubsystem IsPOIDiscovered so undiscovered POIs stay hidden — exploration value preserved) |

| 123 | ASTRAWILD.PCR4.TierBLibrary | PCR-4/PCR-5 (PG-4): Tier-B archetype mesh library — the **36-species size pin** (v9.3 ASSET OVERHAUL: 33 Tier-B species with UNIQUE REAL CC0 meshes + the 3 production bosses DrownedSovereign/EyeSentinel/GlassTyrant, deterministic theme-queue draw; the superseded species render via the real 16-base geometry + unique mutation spec), the convention-path derivation (mesh + idle/move clip paths from the species id — the definition-driven opt-in binding that needs ZERO engine-side code patching), production-boss membership asserts, superseded-species exclusion assert (Echo_Wavecrest), on-disk GLB existence for every entry (the manifest mirror), and the registry binding round-trip after ProductionContent::BuildAll (every Tier-B species carries the convention soft path; explicit art rows always win; unresolved loads fail closed to the PMC body — the boss opt-in contract) |

| 124 | ASTRAWILD.PCR5.HuntSystem | PCR-5 (PG-5): post-game hunt system — the 8-contract table (every row reuses an EXISTING species + reward item; required counts + fail-closed unknown-id lookup), the additive FAstrawildHuntSaveRow (fresh rows pristine; PlayerKey/HuntId/Defeats fields), the wiring surface (controller ToggleHuntScreen/IsHuntOpen/Request+ServerClaimHunt, character HuntAction, pause-menu HuntButton, subsystem GetHuntProgress/IsHuntComplete/ClaimHunt, save object Hunts array) — the post-game "hunts keep running" claim now has a system behind it |

| 125 | ASTRAWILD.FPP1.PresentationContract | FPP-1: player-facing presentation contract — the pure description/cooldown table for all 8 player skills (Masterwork flags itself PASSIVE; None resolves empty), the crafting screen stays CONCRETE (never Abstract again — CreateWidget-instantiable), and the ability library census (53 abilities, every one with display name + description) |

| 126 | ASTRAWILD.SCI_FANTASY.MutationSystem | SCI directive: mutation system contract — table census 204 (one row per bestiary species, unique ids, hero species derive instead), all 8 Sci-Fantasy themes populated, every BaseMeshId ∈ the 16 SK_Base_* archetypes, scale/mask/sound-set/tint field validity, signature diversity ≥ 170/204 (bucketed theme+base+scales+attachments+material — actual 202/204), deterministic BuildDeterministicSpec (identical re-derivation), per-instance salt jitter + root jitter ranges, attachment bit mapping + HasAttachment, theme color math (opaque, in-range, visibly different), derived engine paths (base mesh / clips / element VFX / sound cues), **material-theme master paths (v9.1 §8b: all 8 EAstrawildMutationMaterialTheme values map to 8 DISTINCT M_SciFi_* full-package refs — the runtime swap surface)**, display names for all vocabulary values, and theme×plan bucket resolution over all 64 combinations |
| 127 | ASTRAWILD.DCP1.PostGameQuests | DCP-1 (user directive: re-open deferred work, playable-first): the SQ-23 post-game side-quest batch — five standalone quests (PostVigil/PostFieldNotes/PostGlassTrade/PostDeepRecords/PostLongWatch) mirroring BuildPostGameQuests, pairwise-distinct ids, objectives + research policy, the MQ-17-terminus post-game gate (Quest_FirstDawnAgain), five pairwise-distinct one-time NPC offer flags (Maren/Wren/Tam/Nima/Kael — the Maren crown-choice ForbiddenFlag/SetFlag discipline), the single-active-quest terminus contract (no auto-chaining — a side quest never steals the active slot), objective-vocabulary distinctness (ObserveEcho/CaptureEcho/VisitZone/DiscoverPOI/SurviveTime/DefeatCreature), and exactly-one time-based quest |
| 128 | ASTRAWILD.DCP2.NewGamePlus | DCP-2 (user directive: re-open deferred work): New Game Plus contracts — the world-free scale math (+10% hostile HP/ATK per counted cycle, capped at 5; negative cycles fail closed to 1.0), the research multiplier (+15% per counted cycle, same cap; monotonic to the cap, flat beyond), policy pins (5 counted cycles, 3 carried Echoes), the additive save field round-trip (NGPlusCycle default 0, schema stays v5 — legacy saves load as fresh runs), the carryover-policy mirror (5 kept surfaces: attributes/journal/affinity/defeat-counters/echoes; 18 reset surfaces: quests/research/inventory/survival/durability/spoilage/flags/zones/POIs/events/hunts/buildings/sites/robots/drones/dungeons/ending+postgame/co-op — world seed kept by design), and the research chokepoint rounding rule (Max(base, round(base x multiplier)) — scaling never loses points) |
| 129 | ASTRAWILD.DCP3.EndingCinematic | DCP-3 (user directive: re-open deferred work): the pure-C++ ending cinematic contract — monotonic 9-stop timeline (bars→shotA→title→shotB→subtitle→shotC→hint→fade→restore, 18.7s total, sub-20s respect window), the skip-fold rule (any skip jumps forward to the fade-out stop and shares the SINGLE exit path — view-target/input restore never bypassed), both-ending title-card existence (2 canon verdicts + None), the two entry paths (server/host OnEndingTriggered broadcast + 0.5s client replicated-state poll, both funneling into ONE guarded StartCinematic), and the presentation-only non-negotiable (state transitions route through SetEndingState BEFORE the sequence; the persistent HUD ending banner survives the Z-50 overlay) |
| 130 | ASTRAWILD.DCP4.ActThreeNPCs | DCP-4 (user directive: re-open deferred work): the Act 3 presence pair — Vess (NPC_Vess/Dialogue_Vess, storm-scholar) + Ione (NPC_Ione/Dialogue_Ione, relic trader) exist as distinct ids/trees; distinct REAL bodies via the additive VisualMesh/VisualIdleAnimation/VisualMeshScale fields (T2_Astraite vs T1_Scavenger staged survivor soft paths — neither the player's exosuit tier; fail-closed to the procedural silhouette pre-import); census pins 13 NPCs / 13 trees / 10-roster Dawnstead (updated with registry+validator+docs together); distinct one-time flags (Vess_SurveyTip / Ione_GiftGiven); lore gates on the REAL MQ chain completions (StormAnchors/TheDrownedSovereign/FirstDawnAgain); fresh-definition legacy defaults (null soft paths, unit scale — the 11 classic NPCs untouched) |
| 131 | ASTRAWILD.DCP5.JournalDetailAndToast | DCP-5 (user directive: re-open deferred work): toast cues + per-species journal detail — the single-hook toast contract (PushNotification funnels every ≈60 call site; A_UI_Confirm via the literal-path LoadObject idiom; fail-closed: a missing cue never gates the toast), the pure BuildSpeciesDetailText knowledge-gating contract (bare entry: stats/habits yes, weakness/habitat/food hidden — the exploration loop keeps its value; studied entry: full reveal; null definition: the unresolved fallback), and the view-swap mirrors (list ↔ detail in one region; unknown rows stay cheap TextBlocks, discovered rows the clickable row widget) |
| 132 | ASTRAWILD.DCP6.GamepadSmartCastChord | DCP-6 (user directive: re-open deferred work): gamepad smart-cast chord — the chord contract (LB+X via UInputModifierChordAction; plain X stays Dodge and plain LB stays Block — the modifier disambiguates, no committed binding changed; 17th gamepad mapping), the additive KB/M contract (Y unchanged — one ladder, two devices), and the input-agnostic handler contract (CastPlayerSkill priority ladder: SecondWind > Whirlwind > PowerStrike > HuntersFocus > Dash > Overcharge, 6 rungs, loadout-narrowed) |
| 133 | ASTRAWILD.DCP7.MeshCoverageExpansion | DCP-7 (user directive: re-open deferred work): unique-mesh coverage expansion — the explicit-art table 6→15 (9 boss-spare binds: Lumewisp←SkyTyrant, Gloomfang←WarlordOrc, Sprigling←MushroomKing, Voltmaw←MechRavager, Auroraling←AncientWarden, Wavecrest←AbyssDemon, Undertowray←VoidPrime, Voidwing←ShadowNinja, Verdantbloom←FungalSovereign), 1:1 species↔mesh (the palette-swap guard extended), Printf-convention paths (staged soft paths out of the literal validator set — the Tier-B idiom, fail-closed pre-import), 5 spares stay unbound (headroom), full registry round-trip through BuildAll (all 15 win precedence, Wavecrest + Lumewisp carry direct real meshes) + the PCR4 latent 39-assert fixed (count now derives from the 36-row table — was a v9.3 stale leftover that would have failed the first real engine run) |
