...
| 123 | ASTRAWILD.PCR4.TierBLibrary | PCR-4/PCR-5 (PG-4): Tier-B archetype mesh library — the 39-species size pin (the strategy §5 rule over the ACTUAL source tables), the convention-path derivation (mesh + idle/move clip paths from the species id — the definition-driven opt-in binding that needs ZERO engine-side code patching), on-disk GLB existence for every entry (the manifest mirror), and the registry binding round-trip after ProductionContent::BuildAll (every Tier-B species carries the convention soft path; explicit art rows always win; unresolved loads fail closed to the PMC body — the boss opt-in contract) |
