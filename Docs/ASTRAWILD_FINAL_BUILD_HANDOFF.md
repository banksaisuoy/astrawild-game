...
git lfs pull
python Scripts/validate_final_run.py     # ALL static checks must PASS (104-test gate + 15 census equality gates included)
...
```
**PASS = 104/104 `Result={Success}`, 0 `Result={Fail}`** (count read from the repo — the validator gate pins the exact value). Contracts to watch:
...
- 104 tests have never executed in a real engine (the audit's C-1 drone fix removed a likely
...
- **Drift tripwires**: the validator's census equality gates (15 content-count
  contracts + the exact 104-test gate) fail loudly if a pipeline stage ever
...
3  Test.ps1 104/104 (§9)                           → raw log Docs/ENGINE_LOGS/raw/AUTOMATION_<sha>.log
...
6. Automation now expects **104/104** (was 72 → 84 at GDP → 99 at SCP → 102 at FCR → 103 at DP-3 → 104 at DP-4; the validator gate enforces the exact value — always read the count from the repo, never from memory).
