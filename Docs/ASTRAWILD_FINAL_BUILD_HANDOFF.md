...
git lfs pull
python Scripts/validate_final_run.py     # ALL static checks must PASS (117-test gate + 15 census equality gates included)
...
```
**PASS = 117/117 `Result={Success}`, 0 `Result={Fail}`** (count read from the repo — the validator gate pins the exact value). Contracts to watch:
...
- 117 tests have never executed in a real engine (the audit's C-1 drone fix removed a likely
