...
| M-7 | ~~Dungeon state not saved (reload resurrects encounters); loot granted to first player only (MP-fragile); no entrance/gates~~ **CLOSED in Batch 6** — `FAstrawildDungeonSaveData` additive v2 payload saves cleared-room indices per `DungeonId`; `LoadWorld` applies it after generation (`ApplySavedState` → `RestoreClearedState` silently despawns regenerated encounters, no double loot/events; gates reopen in sync). Entrance/gates: `AAstrawildDungeonPortalActor` pair (teleports + first `Event.LocationReached` publisher) and `AAstrawildDungeonGateActor` (sealed-until-previous-room-clears). First-player-only loot remains (single-player-first; revisit in the MP batch). See `Docs/ASTRAWILD_DUNGEON_BOSS.md` | `DungeonGeneratorActor.cpp` (ExportForSave/ApplySavedState), `SaveSubsystem.cpp`, NEW gate/portal actors |
