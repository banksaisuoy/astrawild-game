...
| High-risk integration and handoff | `validate_mecha_contracts.py`, `validate_vertical_slice_guards.py`, `validate_editor_automation.py`, `validate_vehicle_contracts.py`, `validate_handoff_contracts.py` | Mecha/vehicle CSV/source, gameplay C++, importer/scaffold, docs/runners | Authority/RPC source markers, first-loop guards, 38 CSV mappings, vehicle/garage checks, report/gate alignment และ explicit UE evidence boundaries |
...
**Pass condition:** The script prints `ASTRAWILD Editor automation contract validation passed (38 CSV mappings).`
...
**Pass condition:** The script prints the current 38 CSV, 218 Echo, 2 character, 4 map-kit, 31 SFX, 9 ambience and 2 music source counts.
...
### 3.12 `validate_vehicle_contracts.py`

This narrow Sprint 3 validator requires exactly 12 unique vehicle rows and exactly 12 unique modular part rows. It checks the 12 reflected vehicle types, four modular slots, positive tuning ranges, the compatibility data header and the operational vehicle API, including server-authority control input and replicated state markers.

**Pass condition:** The script prints `ASTRAWILD vehicle contract validation passed (12 vehicles, 12 modular parts, API and authority guards).`

### 3.13 `validate_handoff_contracts.py`

This validator is the final documentation/automation consistency gate. It checks that the handoff, importer, scaffold, PowerShell runners and all validators exist; that the source directory contains exactly 38 CSVs; that `TABLE_MAPPING` equals the source CSV set; that every validator appears in both the handoff and PowerShell runner; that all four JSON reports are produced/referenced; that importer/scaffold operational markers exist; that UE compile/package command markers exist; and that the validator order is deterministic inside `$pythonValidators` rather than being confused by required-file lists.
...
## 5. Space Flight, Guild and Dye contracts

`FAstrawildSpaceFlightData` and `UAstrawildSpaceFlightSubsystem` provide a server-side launch-pad registry, launch progression, low-gravity scale, per-pilot velocity state, vacuum pressure loss and `VacuumEmergency` state. `AAstrawildLaunchPad` registers its location and destination tag. These checks do not prove a custom movement mode, six-degree-of-freedom collision, map travel, pressure damage, oxygen equipment or flight replication.

`FAstrawildGuildData` and `UAstrawildGuildSubsystem` provide server-side guild registration, prerequisite-gated buff levels, territory totem capture and a two-team/four-member arena roster contract. `AAstrawildGuildTotem` registers a territory location and forwards capture requests. These checks do not prove guild persistence, matchmaking, PvP damage, respawn, HUD replication or anti-cheat beyond source authority guards.

`FAstrawildDyeRow` and `DT_Dyes.csv` provide exactly 16 original dye rows with primary/secondary linear tint values, material parameter names, unlock tags, craft costs and default-unlock flags. They do not prove dynamic material instance application, equipment slot compatibility, save migration, cosmetic replication or UMG.
