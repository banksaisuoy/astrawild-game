...
**Status**: SOURCE-VERIFIED readiness matrix. **Nothing in this document is `BOUND`, and no engine-verified or game-ready claim is made — those statuses are reserved for the Antigravity engine run and are not claimed anywhere below.** Statuses separate three trust levels: **SOURCE-VERIFIED** (acquired/authored + validated on disk in this sandbox), **STATIC-VALIDATED** (native content that is code-registered + machine-checked by the 102-test gate and the 15 census gates — but has never been rendered in an engine), and **ENGINE-UNVERIFIED** (applies to every row: import, binding, cook and PIE belong exclusively to the Antigravity one-time integration run per `ASTRAWILD_FINAL_BUILD_HANDOFF.md` §20/§20b).
**Origin**: user directive (2026-09-05) — *"For every acquired asset ask: WHERE IS THIS USED? If the answer is nowhere, do not prioritize importing it."* Registry §I row DP-2.
**Baseline**: `final-completion` @ `981250d` (post DP-1 creature strategy).
...
**Acquisition baseline (authoritative — `ASSET_ACQUISITION_REPORT.md` §2/§4)**: 15 Kenney CC0 packs · **3,678 accepted / 3,360 IMPORT_READY / 75.8 MB**. IMPORT_READY composition on disk: **301 WAV + 657 GLB + 2,396 PNG + 4 FBX + 2 TTF**. Native project-authored ArtSource (flat folders, auto-imported by the unchanged 112-asset contract): **36 WAV + 44 PNG + 32 GLB = 112 files**.
...
- **Native numbers** re-verified on disk: 36 flat WAV + 44 flat PNG + 32 Meshes GLB = **112** (the unchanged `import_all.py` auto-import contract); `Content/` carries 414 `.uasset` + 2 `.umap` committed via LFS.
- **Census facts** (229 species / 12 zones / 3 dungeons / 4 bosses / 11 NPCs / 2 villages / 8 weapons / 26 buildings / 102 tests) are the validator-enforced values — `validate_final_run.py` census gates re-ran PASS at this commit.
- **Placeholder evidence**: boss Cone (`AstrawildEchoBossCharacter.cpp:40-56`), NPC Cylinder+Sphere (`AstrawildNPCCharacter.cpp:48-61`), 3 `NS_AW_*` Niagara systems (`Content/VFX/`), text-glyph crosshair (pure-C++ `HudWidget`).
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **PLAYER** | `SK_Survivor_Exosuit` GLB + 7 anim clips (Aim/Fire/Gather/Idle/Jump/Run/Walk) + `T_Survivor_*` texture set; `.uasset` committed (LFS) at `/Game/Characters/Survivor`; 28-action input map, 5 attributes, 7 milestone skills | none source-side (player body art exists; ACS below is an NPC-side candidate, not a player replacement) | ArtSourceGen (self-authored) | project-authored (no third-party license) | flat contract → `/Game/Characters/Survivor` (part of the 112-asset import) | `ConstructorHelpers` hard refs in `PlayerCharacter` (code-registered) | capsule + `AssetFallback` chain (zero-asset boot guaranteed) | **STATIC_VALIDATED** |
| **ECHOS** | 229 species (census-locked): 6 hero GLB (`SK_Echo_*`) with Idle/Move/Hit clips + `T_Echo_*` 4-slot texture set; 223 procedural PMC bodies (8 body plans × 5 size classes × tint pairs + rarity ring + element point-light) | 8 bespoke Tier-A meshes (4 bosses + Gloomfang/Lumewisp/Sprigling/Auroraling — DP-1 P0) + Tier-B archetype rig library (DP-1 P1) | ArtSourceGen (self); **no external 3D-creature source usable** (Kenney creature packs are 2D; Quaternius deferred/QAL — strategy §2) | project-authored | `ArtSource/Meshes/Echoes/` → `/Game/Characters/Echoes` (existing flat contract) | `AstrawildArtPack::GetEchoArt()` soft refs + automatic PMC fallback | procedural PMC body (spawn never breaks) | **STATIC_VALIDATED** (procedural path) + 6 hero meshes **IMPORT_READY**; Tier-A backlog **MISSING** |
...
| **BIOMES** | 12 zones (per-zone tint/light/height/ridge/threat) + `BiomeDressingActor` (6 canopy styles, per-zone budgets) + 19 native env GLB (trees/rocks/foliage/ruins/4 resource nodes) | per-zone flora variety (cosmetic depth, P2 — not a gameplay gap) | + Kenney Nature Kit (314 GLB) | self + CC0 | → `/Game/Environment/Kenney/NatureKit/` (§20b) | BiomeDressing/POI candidate pools — decided at engine AFTER tone check (ticket 05) | native 19 GLB dressing (already in the 112-asset contract) | **STATIC_VALIDATED** + Nature Kit **IMPORT_READY** |
...
| **DUNGEONS** | 3 dungeons (Hollow Underlight, Sunken Vault, Eye of the Maelstrom — 5 rooms, 400 m altitude) generated procedurally; differ by boss/creature-pool/loot/RP data; **identical room shells today** | per-dungeon room visual identity (tiles) — the visual half of DP-9 | + Modular Space Kit (40 GLB), Modular Dungeon Kit (39 GLB), Space Kit (107 GLB) | self + CC0 | → `/Game/Environment/Kenney/{ModularSpaceKit, ModularDungeonKit, SpaceKit}/` | `DungeonGenerator` room-dressing candidates (engine-side; snap/layout decisions there) | current shell generator | **STATIC_VALIDATED** + 186 tile/dressing GLB **IMPORT_READY** |
...
| **NPCS** | 11 NPCs (affinity tiers 0-100, 4 professions, schedules, shops, 11 dialogue trees) on **Cylinder + Sphere engine-shape placeholder bodies** | NPC body meshes (P1 gap — Kenney 2D rejected; candidate acquired, unbound) | + Kenney Animated Characters: Survivors (1 humanoid FBX + idle/run/jump clips) | self + CC0 | → `/Game/Characters/Kenney/Survivors/` (SkeletalMesh + 3 anims) | retarget compatibility vs `SK_Survivor_Exosuit` = **explicit §20b step-2 check** (rig mismatch is a valid finding, not a source defect) | cylinder+sphere placeholder (current) | systems **STATIC_VALIDATED**; ACS **IMPORT_READY**; bodies **ENGINE_UNVERIFIED** |
| **BOSSES** | 4 boss fights fully coded (Underlight Warden, Dawnfang, Glass Tyrant, Drowned Sovereign): 3 phases + enrage + summons + ×2 weak points + telegraphed AoE + arena hazards | **boss meshes — all 4 render as the engine Cone placeholder** (highest-visibility gap in the game; DP-1 P0 backlog) | 4 bespoke ArtSourceGen meshes to be generated (`gen_boss_*.py` — source-side backlog; no external source usable) | project-authored (when generated) | `ArtSource/Meshes/Echoes/Bosses/` → `/Game/Characters/Echoes` | swap cone → mesh only after import confirmed; cone stays as fallback (same never-auto-replace rule as weapons) | Cone placeholder (current) | fight logic **STATIC_VALIDATED**; boss meshes **MISSING** (source-side) |
...
| `STATIC_VALIDATED` **+** acquired `IMPORT_READY` layer | ECHOS (6 hero GLB) · WEAPONS (Blaster) · BIOMES (Nature) · VILLAGES (Survival) · DUNGEONS (186 tiles) · VFX (96 sprites) · UI (690+2+reticle subset) · AUDIO (301 WAV) · WATER (5 skies) · NPCS (ACS candidate) | acquired packs sit beside working fallbacks — upgrades, not dependencies |
| `MISSING` (source-side backlog, no acquirable source) | **BOSSES meshes (P0)** · **ECHOS Tier-A 8 meshes (P0) + Tier-B rig library (P1)** · NPCS bodies (P1, pending ACS retarget) · UI art until bound | closed only by ArtSourceGen work per DP-1, or by engine binding of acquired candidates |
...
| Creature/boss meshes | **P0 visual** (not closable by acquisition — structural, per DP-1) | none acquirable (Kenney 2D; Quaternius deferred/QAL) | import via `/Game/Characters/Echoes/` + `GetEchoArt()` binding + cone replacement only after import confirmed | **8 bespoke Tier-A meshes (4 bosses first) + Tier-B archetype rig library — ArtSourceGen backlog per `ASTRAWILD_CREATURE_VISUAL_STRATEGY.md` §10** | **MISSING (source-side)** — the one gap acquisition cannot close |
...
| D7 | Boss mesh generation priority (4 bosses first per DP-1 P0) | GLM (source-side backlog) | none — ready to execute as ArtSourceGen batch | cone replacement (after import) |

---

### Integration sequence reminder (unchanged contracts)

1. **Baseline first**: run the standard `import_all.py` pass (112-asset procedural contract) BEFORE any Kenney import — soft-path fallbacks + zero-asset boot must stay intact (§20b step 1).
...
*End of DP-2. One canonical file — per-pipe detail lives in the companion docs referenced in the header.*
