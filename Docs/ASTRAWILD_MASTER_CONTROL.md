...
**Document Version**: 9.7 (VISUAL EXPERIENCE PASS — user FINAL PRODUCT VISION directive: the final experience must read as a MODERN, HI-TECH, COLORFUL sci-fi creature RPG with CUTE creatures (Aniimo-class experience quality as reference class, never cloned — ASTRAWILD keeps its own frontier identity). The v9.6 "source-side complete" freeze is SUPERSEDED by this explicit product-scope expansion: the branch re-opens for VIS-001 only (batches VIS-0..VIS-4, registry §Q + `Docs/ASTRAWILD_LIVE_EXECUTION_STATE.md` queue), then re-freezes at VIS closure. Canon/census/architecture/license constraints inherited UNCHANGED: census 15 gates stay EXACT (229 species / 204 bestiary / 22 quests / 13 NPCs...), no second AI/bond/traversal/skill/boss/mutation architecture, CC0/self-generated assets only, TRUE_MISSING stays 0. Research pass complete (creature-collector experience qualities: visible creature personality, habitat identity, bond + traversal utility, home, profile-first charm). Prior v9.6 record:)
...
**Overall status (v9.7): IN PROGRESS — VIS-001 (user-directed scope expansion: modern cute sci-fi creature/world experience pass on top of the v9.6 complete base; FPP-era freeze text below is historical). On VIS closure the branch re-declares SOURCE_PRODUCT_FROZEN and the only remaining work returns to the one-time Antigravity engine integration (§20 + §20b-e + §22 + V2-36).**
