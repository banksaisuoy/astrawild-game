...
## 1. Complete Keybinding Table (16 keys → 15 actions)
...
| **Right Mouse Button (hold)** | Block (`AWD_Block`) | Started / Completed / Canceled | `StartBlock` / `StopBlock` | Combat (65 % mitigation, ×0.45 move speed) | |
...
prefix commands with `AW.`. All IDs are the registry ids from the Asset Manifest.
