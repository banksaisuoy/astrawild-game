...
| Space flight/lunar | `UAstrawildSpaceFlightSubsystem`, `FAstrawildSpaceFlightData` and `AAstrawildLaunchPad` provide low-gravity, launch progression and vacuum-pressure source contracts | Content/vertical-slice/handoff validators | CharacterMovement, collision, travel, fuel, oxygen equipment, Lunar map and Network PIE remain pending |
| Guild warfare/arena | `UAstrawildGuildSubsystem`, `FAstrawildGuildData` and `AAstrawildGuildTotem` provide buff prerequisites, territory capture and 4v4 roster/match source contracts | Content/vertical-slice/handoff validators | Persistence, PvP damage, respawn, matchmaking, HUD replication and arena map remain pending |
| Cosmetic dyes | `FAstrawildDyeRow` and `DT_Dyes.csv` provide 16 original dye rows with primary/secondary tints, unlock tags and craft costs | Content/editor/handoff validators | Dynamic materials, equipment slots, UMG, save and cosmetic replication remain pending |
| Fishing and racing | `FAstrawildFishingComponent` with FishDex 30-row bait/depth/tension contract; `FAstrawildRacingSubsystem` with ordered server checkpoints, lap timing and distance-validated boost pads | Vertical-slice/handoff guards | Fishing rod/water-volume/economy UI, fish presentation, CharacterMovement speed application, track collision, respawn and leaderboard persistence require UE runtime integration |
...
| Import/scaffold | 38 CSV mappings plus generated OBJ/audio discovery and reports; concrete source counts/destinations/manifests and handoff order checks | Importer coverage + handoff contract validators | Actual execution requires Unreal Editor Python and reflected classes |
...
Compile `ASTRAWILDEditor Win64 Development` with UE 5.8 and MSVC 2022. Fix every UHT, link, Blueprint nativization, and reflected-property issue before importing or authoring content. Run the importer inside the Unreal Editor Python environment and retain its JSON reports. Verify that all 38 DataTables resolve to the expected reflected row structs and that the generated asset registry is saved. Confirm `DT_FishDex` contains 30 rows, `DT_Dyes` contains 16 rows, and that Space Flight launch-pad plus Guild arena/totem registration is performed by authoritative actors or subsystem bridges.
