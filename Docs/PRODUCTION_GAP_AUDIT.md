...
| Import/scaffold | 34 CSV mappings plus generated OBJ/audio discovery and reports; concrete source counts/destinations/manifests and handoff order checks | Importer coverage + handoff contract validators | Actual execution requires Unreal Editor Python and reflected classes |
...
Compile `ASTRAWILDEditor Win64 Development` with UE 5.8 and MSVC 2022. Fix every UHT, link, Blueprint nativization, and reflected-property issue before importing or authoring content. Run the importer inside the Unreal Editor Python environment and retain its JSON reports. Verify that all 34 DataTables resolve to the expected reflected row structs and that the generated asset registry is saved. Confirm `DT_FishDex` contains 30 rows and that Racing track/checkpoint/boost registration is performed by an authoritative actor or subsystem bridge.
