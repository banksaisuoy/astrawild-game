...
| Space flight/lunar source contracts | `UAstrawildSpaceFlightSubsystem`, `FAstrawildSpaceFlightData` and `AAstrawildLaunchPad` provide server-side launch-pad registration, low-gravity scale, launch progression, per-pilot velocity and vacuum-pressure emergency state. CharacterMovement, collision, map travel, fuel, oxygen equipment and Network PIE remain pending. |
| Guild warfare/arena source contracts | `UAstrawildGuildSubsystem`, `FAstrawildGuildData` and `AAstrawildGuildTotem` provide server-side buff prerequisites, territory capture and exactly-two-team/four-member arena roster rules. Persistence, PvP damage, respawn, matchmaking and replicated HUD remain pending. |
| Cosmetic dye source contract | `FAstrawildDyeRow` and `DT_Dyes.csv` provide 16 original dye rows with primary/secondary tints, material parameter names, unlock tags and craft costs. Dynamic material, equipment slots, UMG, save and cosmetic replication remain pending. |
| Fishing/racing source contracts | `FAstrawildFishingComponent` with 30-row FishDex and inventory-safe catch commit; `FAstrawildRacingData`/`UAstrawildRacingSubsystem` with server-owned ordered checkpoints, lap timer and distance-validated boost pads. Fishing rods, water volumes, fish assets, mount speed application, track collision, race UI and leaderboard persistence remain UE/runtime integration work. |
...
| Sprint 3 world and vehicle source contracts | `UAstrawildDisasterSubsystem` covers Meteor Shower, Tornado, Volcanic Ash and Aurora scheduling; `DT_WorldKaijuBosses.csv` covers Magmatitan, SkyColossus and AbyssalLeviathan; `UAstrawildVehicleComponent` exposes server-authority enter/exit, input, nitro, weapon, fuel/health state and modular slot installation for 12 vehicle types and 12 parts. These remain source contracts pending UE compile, DataTable import and runtime/Network PIE verification. |
...
The reviewable source-of-truth CSVs are under `Content/Astrawild/Data/Source/`. The current inventory is 38 tables, including world-event, ecosystem, cooking, progression, underwater, fishing, dyes, space-flight, guild/arena, world-kaiju and multi-terrain vehicle/modular-garage source contracts. `Scripts/validate_handoff_contracts.py` checks that this runbook, the PowerShell runners, importer, scaffold and validator order remain synchronized. Detailed validator behavior is documented in `Docs/VALIDATION_CATALOG.md`.
