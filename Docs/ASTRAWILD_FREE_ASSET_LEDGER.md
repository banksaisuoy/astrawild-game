...
| S-2b | Sci-Fantasy theme master materials (engine-authored by the import, v9.1: 8 not 6) | authored at import time → `/Game/Materials/M_SciFi_*` | 8 parameterized masters (MetallicRobot/EnergyBody/StonyGolem/Slime/Chitin/VoidFlesh/OrganicHide/FocusCrystal — one per EAstrawildMutationMaterialTheme; Tint/PatternTint/GlowIntensity/Metallic/Roughness) — consumed at RUNTIME by `FAstrawildEchoMutator::ApplyThemeMaterial` (dynamic instances on the skinned path); coverage REQUIRED in echo_base_report.json | 8 engine packages | IMPORT_PENDING (V2-35) — counted in report coverage, missing = ERROR |
