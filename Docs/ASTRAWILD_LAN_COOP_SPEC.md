...
| LCP-5 | Client state sync: QuestComponent replication, research + roster mirrors on GameState, client notifications | **DONE** (+2: LCP5.ClientStateSyncSurface / LCP5.ResearchMirrorRoundTrip; roster mirror deferred at LCP-5 — **PCR-2 superseded this exception**: the roster screen now exists and each remote PlayerController carries a replicated per-player RosterMirror slice pushed by the host at capture/remove/evolve/import/late-join; bench/deploy mutations route through ServerSetEchoBenched, never client-authoritative) |
